...
try:
  ...
  DiffusersFlux2KleinPipeline = None
...
class TestFlux2KleinImageEditE2EParity(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.model_dir = get_model_snapshot_dir("black-forest-labs/FLUX.2-klein-4B", "FLUX2_KLEIN_4B_MODEL_PATH")
  ...
  def test_e2e_image_edit_parity_vs_diffusers(self):
    ...
    max_logging.log("\n" + "=" * 80)
    max_logging.log("[STEP 1/3] Running Reference PyTorch Diffusers CPU Pipeline...")
    max_logging.log("=" * 80)
    ...
    max_logging.log(f" -> Saved PyTorch Diffusers output to: {diffusers_img_path}")
    ...
    max_logging.log("\n" + "=" * 80)
    max_logging.log("[STEP 2/3] Running MaxDiffusion Unified FlaxFlux2KleinPipeline on TPU...")
    max_logging.log("=" * 80)
    ...
    max_logging.log(" -> Loading NNX Transformer weights...")
    rngs = nnx.Rngs(0)
    ...
    # 3. Load NNX VAE
    max_logging.log(" -> Loading NNX VAE weights...")
    ...
    # 4. Load Qwen3
    max_logging.log(" -> Loading Qwen3 weights...")
    ...
    # 8. Run pipeline
    max_logging.log(f" -> Running FlaxFlux2KleinPipeline with {len(self.ref_images)} reference images on TPU...")
    pipeline(
        prompt=PROMPT,
        params=t_params,
        vae_params=v_params,
    ...
    max_logging.log("\n" + "=" * 80)
    max_logging.log("[STEP 3/3] Evaluating End-to-End Parity (SSIM & PSNR)...")
    max_logging.log("=" * 80)
    ...
    max_logging.log(f" -> SSIM (Diffusers CPU vs MaxDiffusion TPU): {ssim_val:.6f}")
    max_logging.log(f" -> PSNR (Diffusers CPU vs MaxDiffusion TPU): {psnr_val:.2f} dB")
    max_logging.log(f" -> MSE: {mse:.4f}")
    ...
    max_logging.log(f" -> Saved side-by-side comparison to: {comparison_path}")

    self.assertGreaterEqual(ssim_val, 0.75, f"SSIM score {ssim_val:.4f} is below target threshold 0.75!")
    max_logging.log("END-TO-END PARITY TEST PASSED! MaxDiffusion matches Diffusers reference!")
