...
from maxdiffusion.max_utils import create_device_mesh, get_flash_block_sizes, device_put_replicated
...
def get_model_snapshot_dir(model_id: str, env_var: Optional[str] = None) -> str:
  """Locates the snapshot directory for the given model_id strictly under HF_HOME or explicit env var."""
...
class TestFlux2KleinKVPipelineE2EBF16Parity(unittest.TestCase):
  ...
  @classmethod
  def setUpClass(cls):
    cls.model_path = get_model_snapshot_dir("black-forest-labs/FLUX.2-klein-9b-kv", "FLUX2_KLEIN_KV_MODEL_PATH")
  ...
  @pytest.mark.skipif(
      IN_GITHUB_ACTIONS or Flux2KleinKVPipeline is None,
      reason="Requires TPU, full FLUX.2-Klein 9B weights, and Flux2KleinKVPipeline support in diffusers",
  )
  def test_flux2klein_kv_pipeline_bf16_parity(self):
    ...
    max_logging.log("\n" + "=" * 80)
    max_logging.log("FLUX.2-KLEIN-9B KV CACHE END-TO-END BF16 PARITY TEST")
    max_logging.log("=" * 80)
    max_logging.log(f"Model Path:          {self.model_path}")
    max_logging.log(f"Prompt:              '{PROMPT}'")
    max_logging.log(f"Number of Ref Images:{len(self.ref_images)} (256x256)")
    max_logging.log(f"Target Resolution:   {self.width}x{self.height}")
    max_logging.log(f"Inference Steps:     {self.num_inference_steps}")

    # =========================================================================
    # LEG 1: PyTorch Diffusers Flux2KleinKVPipeline in BF16
    # =========================================================================
    max_logging.log("\n" + "-" * 80)
    max_logging.log("LEG 1: Running PyTorch Diffusers Flux2KleinKVPipeline (bfloat16 on CPU)...")
    max_logging.log("-" * 80)
    ...
    max_logging.log(f" -> Saved PyTorch Diffusers BF16 output: {pt_output_path}")
    ...
    # =========================================================================
    max_logging.log("\n" + "-" * 80)
    max_logging.log("LEG 2: Running MaxDiffusion FlaxFlux2KleinPipeline with use_kv=True (bfloat16 on TPU)...")
    max_logging.log("-" * 80)
    ...
    # Shard onto TPU HBM
    max_logging.log(" -> Sharding parameters across TPU HBM...")
    with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
      t_params = jax.tree_util.tree_map(device_put_replicated, t_params, transformer_shardings)
      v_params = jax.tree_util.tree_map(device_put_replicated, v_params, vae_shardings)
      nnx.update(vae, v_params)
      q_params = jax.tree_util.tree_map(device_put_replicated, q_params, qwen3_shardings)
    ...
    max_logging.log(f" -> Found MaxDiffusion JAX BF16 output: {jax_output_path}")

    # =========================================================================
    # LEG 3: Compute Parity Metrics (SSIM & PSNR)
    # =========================================================================
    max_logging.log("\n" + "=" * 80)
    max_logging.log("CROSS-FRAMEWORK BF16 PARITY EVALUATION REPORT")
    max_logging.log("=" * 80)
    ...
    max_logging.log(f" -> Structural Similarity (SSIM): {score_ssim:.6f}")
    max_logging.log(f" -> Peak Signal-to-Noise Ratio (PSNR): {score_psnr:.2f} dB")
    max_logging.log("=" * 80)

    self.assertGreaterEqual(
        score_ssim, 0.70, f"End-to-End BF16 SSIM {score_ssim:.6f} is below the required acceptance threshold of 0.70"
    )
    max_logging.log("End-to-End BF16 Parity Test PASSED successfully!\n")
